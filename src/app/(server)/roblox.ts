...
export async function randomRobloxInfo(): Promise<randomRobloxProps>{
...
async function formatDate(inputDate: string){
...
export async function robloxAPI(): Promise<RobloxApiResponse> {
    const filePath = './files/ids.txt';

    return new Promise<RobloxApiResponse>(async (resolve, reject) => {
        ...
        readStream.on('close', async () => {
            if (lines.length !== 0) {
                ...
                try {
                    ...
                    resolve({ count, name, creationDate, id, displayName, imageUrl });
...
export async function sendRobloxLogs(webhook: string): Promise<string | void>{
